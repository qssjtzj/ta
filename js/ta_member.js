    ...
    $(function () {
        ...
        $(window).scroll(function () {
            ...
            if (maxTop < $(window).height() + $(document).scrollTop()) {
                ...
                if (type) {
                    if (type == 'online') {
                        var url = "/index/online_ajax";
                    } else if (type == 'photos') {
                        var url = "/index/photo_ajax";
                    } else {
                        var url = "/index/wxonline_ajax";
                    }
                }
...
/!*
                ...
                function test_sql(result) {
                    ...
                    for (var j = 0; j < result.length; j++) {
                        //console.log(result);
                        if (type == 'online') {
                            html += '<li class="cell item"><a href="/user/member/id/' + result[j]['id'] + '" target="_blank">';
                            if (result[j]['pic_name'] != '') {
                                html += '<img src="' + result[j]['pic_name'] + '" />';
                            } else {
                                if (result[j]['sex'] == 1) {
                                    html += '<img src="/public/images/man.jpg" />';
                                }
                                if (result[j]['sex'] == 2) {
                                    html += '<img src="/public/images/woman.jpg" />';
                                }
                            }
                            html += '</a><div><h3>' + result[j]['nickname'];
                            if (result[j]['focus']) {
                                html += '<a href="javascript:void(0)" class="qx_btn" data-id="' + result[j]['mid'] + '">取消关注</a>'
                            } else {
                                html += '<a href="javascript:void(0)" class="gz_btn" data-id="' + result[j]['mid'] + '">关注</a>';
                            }
                            html += '</h3><span>' + result[j]['birthday'] + '岁，' + result[j]['workprovince'] + result[j]['workcity'] + ',' + result[j]['profession'] + '</span><em style="margin: 0;border: 0;padding: 0">内心独白：' + result[j]['intro'] + '</em></div></li>';
                        }
                        if (type == 'photos') {
                            html += '<li class="cell item"><a href="/user/member/id/' + result[j]['mid'] + '" target="_blank"><img src="' + result[j]['pic_name'] + '" /></a><div><h3>' + result[j]['nickname'] + '</h3><span>' + result[j]['birthday'] + '岁，' + result[j]['workprovince'] + result[j]['workcity'] + ',' + result[j]['profession'] + '</span><div style="margin-bottom: 10px;padding-left: 0">上传时间：' + result[j]['post_time'] + '</div><i class="liked"><input type="button" value="喜欢" class="btn_like" data-id="' + result[j]['id'] + '" id="' + result[j]['id'] + '"></i><em>' + result[j]['like_num'] + '</em>';
                            if (admin == 1) {
                                html += '<button style="width: 70px" class="del_photo" data-id="' + result[j]['id'] + '">删除</button>'
                            }
                            html += '</div></li>';
                            getCookie(result[j]['id']);
                        }
                        ...
                        if (type == 'wxonline') {
                            ...
                            html += '</h3><span>微信号：' + result[j]['weixin'] + '</span><span>' + result[j]['birthday'] + '岁，' + result[j]['workprovince'] + result[j]['workcity'] + ',' + result[j]['profession'] + '</span><em style="margin: 0;border: 0;padding: 0">内心独白：' + result[j]['intro'] + '</em></div></li>';
                        }
                    }
                    ...
                    var time = setTimeout(function () {
                        ...
                        var $newElems = $(html).css({opacity: 1}).appendTo(container);
...
var CONST = {
    edulist: ["高中及以下", "中专", "大专", "大学本科", "硕士", "博士"],
    citylist: [
        {
        "p": "北京",
    ...
    xingzuo: ["白羊座(03.21-04.20)", "金牛座(04.21-05.20)", "双子座(05.21-06.21)", "巨蟹座(06.22-07.22)", "狮子座(07.23-08.22)", "处女座(08.23-09.22)",
        "天秤座(09.23-10.22)", "天蝎座(10.23-11.21)", "射手座(11.22-12.21)", "魔羯座(12.22-01.19)", "水瓶座(01.20-02.19)", "双鱼座(02.20-03.20)"]

}

var member = {
    WaterFull:$("#waterfull ul"),
    limit:20,
    page:1,
    init: function () {
        ...
        member.getlist(this.page, this.limit);

    },
    loadSelect:function(){

        for(var i = 0; i < CONST.xingzuo.length; i++){
            var obj = CONST.xingzuo[i];
            $("#constellation").append('<option value="' + obj +'">' + obj + '</option>');
        }

        for(var i = 0; i < CONST.citylist.length; i++){
            var obj = CONST.citylist[i];
            $("#province").append('<option value="' + obj.p +'">' + obj.p + '</option>');
        }
            ...
            for(var i = 0; i < CONST.citylist.length; i++){
                ...
                if(p === obj.p ){
                    for(var j = 0; j < obj.c.length; j++){
                        $("#city").append('<option value="' + obj.c[j].n +'">' + obj.c[j].n + '</option>');
                    }
                }
            }
        });
...
*/
    ...
    getlist:function(page, limit){
        ...
        var url = TA.apiPath + "/member/search_member.php";
        var data = {page : page, limit : limit};
        $("#w_list1 select").map(function() {
            if(this.value == 0 || this.value == ''){
                return;
            }
            data[this.name.replace("[]","")]= this.value;
        });
        ...
        RequestUtil.credent("GET",url, data , function(data){
            ...
            if (data.rescode == 200 ) {
                ...
                member.WaterFull.html("");

                for (var i = 0; i < objs.length; i++) {
                    var obj = objs[i];

                    var div = '<li class="cell item masonry-brick" style=""> ' +
                        '<a href="/usercenter/user_center.html?id=' + obj.id + '" target="_blank"> ' +
                        '<img src="' + obj.icon + '" width="248" height="248"> </a> ' +
                        '<div><h3> ' + obj.nick + ' <a href="javascript:void(0)" class="gz_btn" data-id="' + obj.id + '">关注</a></h3>' +
                        '<span style="margin-bottom: 10px;">' + obj.age + '，' + obj.work_place + ', ' + obj.occupation + ' </span>' +
                        '<em style="margin: 0;border: 0;padding: 0"></em></div></li>';

                    member.WaterFull.append(div);
                }

                member.load();
            } else {
                member.WaterFull.html("onload error");
            }
        } );
    },
    follow:function(){
