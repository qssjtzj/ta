...
$(document).ready(function(){
    ...
    $(document).on('click', '.btn_like', function () {
        var _this = $(this);
        var photo_id = _this.attr('data-id');
        var url = "/index/photo_likes";
        $.ajax({
            url: url,
            ...
            success: function (data) {
                ...
                if (data.code == 1) {
                    ...
                    //alert(number);
                    _this.removeClass('btn_like').addClass('zan');
                    _this.parent().next().text(number);
                    setCookie(photo_id, photo_id, 30);
    ...
    function getCookie(c_name) {
        ...
        if (document.cookie.length > 0) {
            ...
            if (c_start != -1) {
                ...
                $(id).removeClass('btn_like').addClass('zan');
                // $(id).children().first().attr('src','/public/home/images/sc_z2.png');
                return unescape(document.cookie.substring(c_start, c_end))
            }
        }
        return ""
    }
    //setCookie('dingke','123456',20);
    //var data = getCookie('dingke');

    //遍历所有的喜欢按钮
    function all_like_btn() {
    ...
    //关注
    $(document).on('click', '.gz_btn', function () {
        ...
        $.ajax({
            url: "/user/focus_user",
            type: 'POST',
            data: {focused_user_id: uid},
            dataType: 'json',
            success: function (data) {
                if (data.status) {
                    _this.removeClass().addClass('qx_btn');
                    _this.text("取消关注");
                } else {
                    if (!data.info) {
                        $(".post_ajax").css('display', 'block');
                    } else {
    ...
    $(document).on('click', '.qx_btn', function () {
        ...
        var uid = _this.attr('data-id');
        $.ajax({
            url: "/user/unfocus_user",
            type: 'POST',
            data: {focused_user_id: uid},
            dataType: 'json',
            success: function (data) {
                if (data.status) {
                    _this.removeClass().addClass('gz_btn');
                    _this.text("关注");
                } else {
                    if (!data.info) {
                        $(".post_ajax").css('display', 'block');
