...
window.WebRC = (function() {
    ...
    WebRC.prototype.setup = function(target, swf) {
        ...
        var params = {
            //menu: 'false',
           // quality: 'high',
            wmode: 'opaque', //'window',//transparent
		...
		//if(window.location.href.indexOf("/live/t2.html")>0){
			return swfobject.embedSWF(path, target, '485', lastFlashHeight, '10.1', 'js/expressInstall.swf', vars, params, attrs);
			
		//}
		
		return swfobject.embedSWF(path, target, '485', '730', '10.1', 'js/expressInstall.swf', vars, params, attrs);
	...
	api.rank = {
		...
		getItemHtml: function(obj) {
			...
			var div = '<li><span class="fl rank_num num'+obj.rid+'">'+rid
						...
						+' " width="60" height="60" border="1" onerror="this.src=\'../imgs/icon.png\'"/> ';
			if(obj.guardLevel!='' && obj.guardLevel>0){
				div+=' <img class="rank_guaid" src="img/icon/icon_guard1.png" width="16" height="36" border="0"/>';		
			}
                       
            div+= '<div class="fl rank_count"><div class="name">';
			if(obj.vipLevel!='' && obj.vipLevel>0){
				div+=' <img class="fl rank_vip" src="../imgs/icon_vip.png" width="19" height="18" border="0"/>';		
			}				
...
$(function() {
    ...
    jQwin.on('flash.resize', function(e) {
		...
		if (wh > flashHeightList[2] + topNavHeight +80) {
			size = flashHeightList[2];
		} else if (wh > flashHeightList[1] + topNavHeight +80) {
		...
		//if (size != lastFlashHeight  && window.location.href.indexOf("/live/t2.html")>0) {
            ...
            if (elemFlash.length && elemMsg.length ) {
				...
				tabContents.css('height', s_size + 39 );
                elemMsg.css('height', s_size  - formTextHeight);
				elemCarCont.css('height', s_size);
				scrollerMsg.css('height', scroller_size);
				
				elemMsg2.css('height', private_size);
				elemMsg1.css('height', s_size  - formTextHeight - elemMsg2.height()- msgPaddingTop-20 );
				slide.css('bottom', 60).css('top', elemMsg1.height() + msgPaddingTop - 9);
    ...
    */
    ...
    /*
    //RC invite
    window.rcInviteInit = function() {
        if (!FB) {
            return false;
        }
        var btnFbInvite = $('#btnFbInvite').hide();
        window.fbInvite = new RcInvite();
        fbInvite.onListReady = function() {
            btnFbInvite.show();
        };
        fbInvite.setData(serverData.invite);
        fbInvite.checkLogin = function(cb) {
            FB.getLoginStatus(function(ret) {
                cb.call(fbInvite, ret.status == 'connected');
            });
        };
        fbInvite.startLogin = function(cb) {
        ...
        fbInvite.getList = function(cb) {
            FB.api('me/friends', function(ret) {
                var friendList = [];
                for (var i = 0, len = ret.data.length; i < len; i++) {
                    friendList.push({
                        id: ret.data[i].id,
                        name: ret.data[i].name,
                        face: 'http://graph.facebook.com/' + ret.data[i].id + '/picture?type=square'
                    });
                }
                cb.call(fbInvite, friendList);
            });
        };
        fbInvite.startInvite = function(data, ids, cb) {
            if (ids.length == 1) {
                FB.ui({
                    method: 'send',
                    to: ids[0],
        ...
        fbInvite.init('fbInviteList', 'fbInviteLogin', 'fbInviteError', 'fbInviteLoading');
    };
    */
		...
		$.ajax({
				...
				success : function(data) {
					...
					if (data != null && data.user_success == true) {
						...
						$("#channel_name").html(u.nick+"<span> ("+channel+")</span>");
				...
				if (data != null && data.user_success == true) {
					...
					if(data.list.length>0){
						for(var i in data.list){
							...
							var channel = opencyou.utils.getParamFromUrl("s");
							//console.log("$$$$$$$$$",sid ,uid , window.anchor_id,uid== window.anchor_id,channel == sid);
    ...
    window.rcAsyncInit = function() {
        ...
        RC.Event.subscribe('login', function(ret) {
			rclog("login",ret);
            WEB_RC.conf.guest = false;
            $.getJSON('/index.php?c=user&a=getUserInfo', function(obj) {
                $('#userFace1').attr('src', obj.face);
                $('#userNick1').text(obj.nick);
                $('#pane-login').hide();
                $('#profile').show();
            });
            if (ret) {
                serverData.uid = ret.id;
                serverData.account = ret.account;
                serverData.nick = ret.nick;
                serverData.param = ret.param;
                webRC.connect({
                    uid: ret.id,
                    param: ret.param,
                    guid: serverData.uuid
            ...
            elemTips.hide();
            elemFlower.removeClass('logout');
        });
		
        RC.Event.subscribe('logout', function(ret) {
			rclog("logout");
            WEB_RC.conf.guest = true;
            webRC.uid = serverData.uid = 0;
            serverData.account = serverData.nick = '';
            webRC.param = serverData.param = '';
            webRC.connect();
            $('#profile').hide();
            $('#pane-login').show();
            elemTips.html(txtGet).show();
            if (webRC) {
                webRC.fire('flower.update', {});
            }
            elemFlower.addClass('logout');
        });
        if (serverData.uid && serverData.param) {
            RC.Event.trigger('login', [null]);
        } else {
            elemTips.html(txtGet).show();
        }
    };
	*/
	/*
    (function(d, debug) {
        var js, id = 'showoo-jssdk', ref = d.getElementsByTagName('script')[0];
        ...
        js.id = id;
        js.async = true;
        js.src = 'js/' + (debug ? 'debug' : 'all') + '.js';
        ref.parentNode.insertBefore(js, ref);
    }(document, false));
    */
	/*
    //RC share
    window.rcShare = new RcShare();
    rcShare.init('rcShareContainer', serverData.share);
    */
	/*
    //FB login
    window.fbAsyncInit = function() {
        FB.init({
            appId: 485170458186080,
            channelUrl: 'http://rcshow.tv/channel.html',
            status: true,
            cookie: true,
            xfbml: true
        });
